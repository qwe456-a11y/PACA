...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, faces, texture_size, vertices, img_size=800, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
    ...
    def __getitem__(self, index):
        ...
        # Letterbox
        shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape ：1024
        ...
        # image_NSR_ref = mask*imgs_pred
       
        # filename=self.img_files[index].split('/')[-1]
        # color_ref_path=os.path.join("./src/test/nrp/color",self.color_name,filename)
        # # color_ref_path=os.path.join("./src/texture/train_new",filename)
        ...
        # image_NSR_ref = image_NSR_ref
        # image_NSR_ref = image_NSR_ref*mask
        image_NSR_ref = img / 255.0
        
        return img.squeeze(0), imgs_pred.squeeze(0), mask, img_cut.squeeze(0), image_NSR_ref.squeeze(0),labels_out, self.img_files[index], shapes


    @staticmethod
    def collate_fn(batch):
        #img, texture_img, masks,imgs_ref,img_cut, label, path, shapes = zip(*batch)  # transposed
        img, texture_img, masks,img_cut, image_NSR_ref,label, path, shapes = zip(*batch)
        ...
        return torch.stack(img, 0), torch.stack(texture_img, 0),torch.stack(masks, 0), torch.stack(img_cut, 0), torch.stack(image_NSR_ref, 0),torch.cat(label, 0), path, shapes
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1024)  # BCHW
            i *= 4
...
def load_image(self, index):
    ...
    return img, (h0, w0), img.shape[:2], (obj_location, obj_rotation, cam_location, cam_rotation)  # img, hw_original, hw_resized ，h0，w0是原来的大小，h，w是resize成正方形后的大小
