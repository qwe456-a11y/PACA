...
# 创建用于训练的数据加载器，以及相关的数据集对象，用于提供图像和标签数据。
def create_dataloader(path, imgsz, batch_size, stride,faces, texture_size, vertices, opt, augment=False, cache=False, pad=0.0, rect=False,
    ...
    # 上下文管理器，确保在分布式数据并行训练中，只有第一个进程处理数据集，其他进程可以使用缓存。
...
# 训练过程中无限循环地使用数据加载器的工作进程，以便在训练过程中能够动态更新数据集属性
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, faces, texture_size, vertices, img_size=800, batch_size=16, augment=False, rect=False, image_weights=False,
        ...
        # self.mask_renderer.renderer.renderer.camera_mode = "look"
        self.mask_renderer.renderer.renderer.viewing_angle = 45
        ...
        self.mask_renderer.renderer.renderer.far = 1000
    ...
    def __getitem__(self, index):
        # 加载图像，并获取图像、原始高度和宽度、缩放后的高度和宽度以及车辆和相机的变换参数
        # img, (h0, w0), (h, w), (veh_trans, cam_trans) = load_image(self, index)
        img, (h0, w0), (h, w), (obj_location, obj_rotation, cam_location, cam_rotation) = load_image(self, index)
        # camera parameters
        # object_pos = np.array([371.341, 40.9655, 0])
        # object_q = np.array([0, 0, 0])
        # camera_pos = np.array([-21.341, -15.9655, 150])
        # camera_q = np.array([-90, 0, 0])
        ...
        # eye, direction, up = nmr.get_params(object_pos, object_q, camera_pos, camera_q)
        # R, t = camera_to_Rt(eye, direction, up)
        # print('obj_location:{}\n'.format(obj_location))
        # print('obj_rotation:{}\n'.format(obj_rotation))
        # print('cam_location:{}\n'.format(cam_location))
        # print('cam_rotation:{}\n'.format(cam_rotation))
        ...
        # render image via neural renderer 得到渲染后的图像
        imgs_pred = self.mask_renderer.forward(self.vertices_var, self.faces_var, self.textures)
        # # Image.fromarray((imgs_pred).data.cpu().numpy()[0].transpose((1, 2, 0)).astype('uint8')).save(
        # #                 os.path.join('./src/test/nrp', 'texture2.png'))
        # Image.fromarray(np.uint8(255 * imgs_pred.squeeze().cpu().data.numpy().transpose(1, 2, 0))).show()

        # 对渲染后的图像进行归一化处理，将像素值缩放到0到1之间
        ...
        # Letterbox 计算图像的最终尺寸，如果使用了矩形训练，则使用批次形状，否则使用图像的原始尺寸
        shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
        # 对图像进行Letterbox处理，将其转换为模型需要的尺寸，并返回缩放比例和填充参数
        ...
        img = img.transpose((2, 0, 1))[::-1]  #[::-1]   # HWC to CHW
        img = np.ascontiguousarray(img)
        img = torch.from_numpy(img).to(self.device)
        img_cut=(img) * mask

        # Applying mask, the transformation function in paper 将图像应用掩码，实现图像和渲染图像的融合

        # print(img.shape, imgs_pred.shape, mask.shape)
        img = (1 - mask) * img + (255 * imgs_pred) * mask
        imgs_pred= mask * imgs_pred
        # imgs_pred= imgs_pred

        # 返回处理后的图像、渲染图像、掩码、标签等信息，以及图像文件名和形状信息
...
def camera_to_Rt(position, direction, up):
    # 步骤 1：规范化方向和上方向
...
def load_image(self, index):
    ...
    # obj_trans, cam_trans = data['veh_trans'], data['cam_trans']
    ...
    return img, (h0, w0), img.shape[:2], (obj_location, obj_rotation, cam_location, cam_rotation) # img, hw_original, hw_resized
    # return img, (h0, w0), img.shape[:2], (K, R, t)  # img, hw_original, hw_resized
